...
// ══════════════════════════════════════════════════════════════════
{
  ...
  // Hub center — r=2.1 prevents bottom-right/bottom node overlap
  const cx = 4.8, cy = 3.7, r = 2.1;
  s.addShape(pres.shapes.OVAL, {
  ...
  nodes.forEach(n => {
    ...
    s.addText(n.name, {
      x: nx - nw / 2 + 0.06, y: ny - nh / 2 + 0.04, w: nw - 0.1, h: 0.24,
      fontSize: 8.5, color: n.color, bold: true, align: 'left', valign: 'middle', margin: 0
    });
    s.addText(n.desc, {
      x: nx - nw / 2 + 0.06, y: ny - nh / 2 + 0.26, w: nw - 0.1, h: 0.22,
      fontSize: 6.5, color: C.t2, align: 'left', valign: 'middle', margin: 0
    });
  });
...
// ══════════════════════════════════════════════════════════════════
{
  ...
  const fmodules = [
    { color: C.emerald, label: 'Career Twin'  },
    { color: C.indigo,  label: 'Sim Lab'      },
    { color: C.amber,   label: 'Explainability'},
    { color: C.indigo,  label: 'Demo Mode'    },
  ];
  fmodules.forEach((m, i) => {
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      x: dx + 0.3 + i * 1.82, y: dy + dh - 0.48, w: 1.55, h: 0.32,
      fill: { color: m.color, transparency: 82 }, line: { color: m.color, width: 1 }, rectRadius: 0.05
    });
    s.addText(m.label, {
      x: dx + 0.3 + i * 1.82, y: dy + dh - 0.48, w: 1.55, h: 0.32,
      fontSize: 7.5, color: m.color, bold: true, align: 'center', valign: 'middle', margin: 0
    });
  });
...
// ══════════════════════════════════════════════════════════════════
{
  ...
  dcards.forEach((d, i) => {
    ...
    s.addText('EVIDENCE:  ', {
      ...
      fontSize: 6.5, color: C.t2, bold: true, align: 'left', valign: 'middle', margin: 0
    });
